...
const handleSingleMessage = async (
  ...
  try {
    logger.info(
      `📩 Message received from ${message.from} | Type: ${message.type} | MsgID: ${message.id}`,
    );
    ...
    if (!tenant) {
      logger.warn(
        `⚠️ No active tenant found for phone_number_id: ${phoneNumberId}`,
      );
      return;
    }
    ...
    if (
      ...
      logger.warn(
        `⚠️ Tenant ${tenant.name} subscription is ${tenant.subscriptionStatus}. Skipping.`,
      );
      return;
    }
