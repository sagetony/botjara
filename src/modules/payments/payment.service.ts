...
export const initializePayment = async (
  ...
  try {
    const reference = `WAAI-${order.orderNumber}-${Date.now()}`;
    const amountKobo = order.total * 100; // Paystack uses kobo

    const response = await axios.post(
      `${PAYSTACK_BASE}/transaction/initialize`,
      {
        email: `${customerPhone}@waai.ng`, // Paystack requires email, we generate one
        amount: amountKobo,
        reference,
        currency: 'NGN',
        metadata: {
    ...
    logger.info(`💳 Payment initialized for ${order.orderNumber}: ${reference}`);
...
export const handlePaymentWebhook = async (
...
): Promise<{ orderId: string; tenantId: string; customerPhone: string } | null> => {
  ...
  try {
    ...
    if (!payment) {
      logger.warn(`⚠️ Payment webhook received for unknown reference: ${reference}`);
      return null;
    }
